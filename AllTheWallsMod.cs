	...
	//		new NewWall(WallID.GraniteUnsafe,"GraniteUnsafe", "Granite Wall (Unsafe)", "Granite_Wall"),
		...
		public override void AddRecipes()
		{
			...
			foreach (var item in convertToUnsafeWalls)
			{
                ...
                CreateRecipie(safeWall, unsafeWall);
                CreateRecipie(unsafeWall, safeWall);
			}
            ...
            for (var color = 0; color < dungeonWalls.Length; color += 3)
            {
                for (var ingredient = color; ingredient < color + 3; ingredient++)
                {
                    ...
                    CreateRecipie(ingredientUnsafeWall, ingredientSafeWall);
                    CreateRecipie(ingredientSafeWall, ingredientUnsafeWall);
                    ...
                    for (var result = color; result < color + 3; result++)
                    {
                        ...
                        CreateRecipie(ingredientUnsafeWall, resultUnsafeWall);

                        //Convert between any safe walls
                        var resultSafeWall = dungeonWalls[result].originalWallItemID;
                        CreateRecipie(ingredientSafeWall, resultSafeWall);
                    }
                }
            }

            void CreateRecipie(int ingredient, int result)
            {
